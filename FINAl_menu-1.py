...
def save_game_players(player1, player2):
    game_data = {"player1": player1, "player2": player2}
    save_to_file(game_data, GAME_PLAYERS_FILE)
    console.print(f"Game players saved: {player1} vs {player2}", style="success")
        ...
        if user in user_names:
            ...
            while True:
                ...
                if bcrypt.checkpw(password.encode(), bytes.fromhex(user_names[user]["password"])):
                    ...
                    save_game_players(first_user, user)
                    return user
                else:
