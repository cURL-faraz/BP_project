...
def endtime (start_time):
    end_time = time.time()
    gametime=int(end_time - start_time)
    h=gametime//3600
    gametime-=h*3600
    m=gametime//60
    gametime-=m*60
    gametime=gametime//1
    print (f"{h}:{m}:{gametime}")
    return(gametime)
def read_player(filepath):
    with open(filepath, 'r') as f:
        data = json.load(f)
    player1 = data.get("player1")
    player2 = data.get("player2")
    return([player1, player2])
def read_update_board(filepath,player,wl):
    if wl == "win":
...
players=read_player("game_players.json")
if red[0]==9:
    print(f"{players[0]} wins")
    read_update_board("boardfile.json",players[0],"win")
    read_update_board("boardfile.json",players[1],"lose")
    
    endtime(start_time)
if blue[0]==1:
    print(f"{players[1]} wins")
    read_update_board("boardfile.json",players[1],"win")
    read_update_board("boardfile.json",players[0],"lose")
    endtime(start_time)
